...
//RETRIEVE ORDERS BASE ON THEIR ORDER STATUS
if($status == "fulfilled" || $status == "unfulfilled" || $status == "cancelled")
  $sql = "SELECT * FROM `order_status` where status = '$status'";
if($status == "Select")
  $sql = "SELECT * FROM `order_status` ORDER BY orderid DESC";

$getorderid = mysqli_query($conn,$sql);

if(mysqli_num_rows($getorderid) > 0){

  while($order = mysqli_fetch_assoc($getorderid)){
